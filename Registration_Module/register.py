...
model_field.load_state_dict(torch.load('checkpoints/2/model_field_epoch_200.pkl'))
model_field.eval()

model_trans = UNet(in_channels=2, n_classes=2, depth=4, wf=4, up_mode='upconv')
model_trans.to(device)
model_trans.load_state_dict(torch.load('checkpoints/2/model_trans_epoch_200.pkl'))
model_trans.eval()

t_model_field = UNet(in_channels=2, n_classes=1, depth=4, wf=5, up_mode='upconv')
t_model_field.to(device)
t_model_field.eval()
t_model_field.load_state_dict(torch.load('checkpoints/2/t_model_field_epoch_200.pkl'))

t_model_trans = UNet(in_channels=2, n_classes=2, depth=4, wf=4, up_mode='upconv')
t_model_trans.to(device)
t_model_trans.eval()
t_model_trans.load_state_dict(torch.load('checkpoints/2/t_model_trans_epoch_200.pkl'))
...
for num in range(0, 21):
    ...
    data_cube = np.fromfile('./reg_discussions/Poseidon_Reg_with_Error/Poseidon_128_128_128_' + str(num).rjust(3, '0') + '.bin', dtype=np.float32)
    ...
    label_mid = plt.imread('./reg_discussions/Poseidon_Reg_with_Error/Label_InLine64_' + str(num).rjust(3, '0') + '.png')
    label_mid = torch.from_numpy(label_mid)
    label_mid = label_mid.cuda()
    label_mid = torch.unsqueeze(torch.unsqueeze(label_mid, 0), 0)

    t_label_mid = plt.imread('./reg_discussions/Poseidon_Reg_with_Error/Label_TimeSlice64_' + str(num).rjust(3, '0') + '.png')
    ...
    # label_cube = np.transpose(label_cube, [2, 0, 1])  # jjk order
    label_cube = np.transpose(label_cube, [1, 2, 0])  # zz order
    label_cube = np.reshape(label_cube, [128 * 128 * 128], 'F')
    label_cube.tofile('./reg_discussions/Poseidon_Reg_with_Error/Label_pseudo_inline_dir_' + str(num).rjust(3, '0') + '.bin')

    # t_label_cube = np.transpose(t_label_cube, [2, 0, 1])
    t_label_cube = np.transpose(t_label_cube, [1, 2, 0])
    t_label_cube = np.reshape(t_label_cube, [128 * 128 * 128], 'F')
    t_label_cube.tofile('./reg_discussions/Poseidon_Reg_with_Error/Label_pseudo_timeslice_dir_' + str(num).rjust(3, '0') + '.bin')
